 ...
 *
 * The contract is intentionally narrow: start() turns the mic on, the implementation calls back
 * with raw bytes (16 kHz mono PCM in production, arbitrary in tests), and stop() turns it off.
 * Lifecycle is one-shot per utterance — a fresh start() call gets a fresh stream.
 *
 * Implementations:
 * - `AndroidMicSource` (shipping path) — wraps Android `AudioRecord` for
 * ```
 *    16 kHz mono PCM capture on stock Android / current Rokid builds.
 * ```
...
/**
 * Test / dev MicSource. Pre-loaded with a list of byte chunks; calling `start` synchronously emits
 * them one by one. Useful for replay tests and for running the agent against a recorded session in
 * CI.
 */
...
/**
 * Placeholder for the privileged CXR-L–backed implementation. The real version will:
 ...
 *
 * Lives here so the orchestration in [MicSession] is hardware-agnostic; swap this for
 * [AndroidMicSource] on current Android builds and for [FakeMicSource] in tests / when running
 * off-glasses.
 */
